---
title: How Xiaohongshu (RedNote) Scales to Handle Traffic Surges with Karmada-Powered Multi-Cloud Federation
---
...
Xiaohongshu's infrastructure evolved into a complex landscape of dozens of Kubernetes clusters spread across multiple cloud providers and self-built data centers. This fragmentation stemmed from several factors:

- **Cluster Size Limitations**: Using managed Kubernetes services (TKE/ACK) from cloud vendors imposed strict cluster size limits. While this ensured individual cluster stability, it created resource silos from a global perspective.
...
The ideal deployment model was straightforward: business teams select a region, and the platform provides a unified resource pool. The reality was starkly different—a single region might contain dozens of isolated clusters with heterogeneous machine types, making efficient resource scheduling nearly impossible.

![Xiaohongshu Architecture](./static/xiaohongshu_01.webp)

### The TikTok Refugee Crisis: A Wake-Up Call

In January 2025, when TikTok faced potential restrictions, millions of users suddenly migrated to Xiaohongshu, causing unprecedented traffic spikes. The self-built data centers' resources were immediately exhausted. The team faced three difficult options:
...
The solution comprised three core components:
...
The scheduling architecture implements a "self-built first, cloud backup" policy:
...
Xiaohongshu extended Karmada with custom capabilities to ensure production-grade reliability:
...
Search and recommendation services at Xiaohongshu process massive index tables requiring stateful orchestration. Native StatefulSets couldn't be split across clusters due to rigid pod naming and indexing.

The custom stateful workload enables:
- Customizable pod indexing across clusters (e.g., cluster 1: pods 0-2, cluster 2: pods 3-5)
- Fine-grained status reporting for each pod index
...
- **Cost optimization**: Service-level granular scaling minimized costs compared to chain-wide scaling
...
Large language model inference presented unique challenges:
...
1. **Start with resource efficiency**: Federation's value extends beyond disaster recovery to fundamental resource optimization
...
The success during the TikTok migration event validated the architecture's core premise: unified resource scheduling across fragmented clusters enables both operational efficiency and business resilience. As Xiaohongshu continues to scale, Karmada remains central to their strategy for managing complexity while maintaining agility in a dynamic multi-cloud environment.
